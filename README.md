...
Booking-service при получении запроса сохраняет заявку в своей БД со статусом `PENDING` и публикует сообщение (команду/событие) в очередь (через Outbox pattern), после чего немедленно отвечает пользователю. Между Booking-service и Restaurants-service вводится очередь сообщений (например, RabbitMQ или Kafka), расположенная в ДЦ1
