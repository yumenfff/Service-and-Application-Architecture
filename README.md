...
| `if/else if` при создании курсов | Иерархия `CourseFactory` с конкретными фабриками и `CourseFactoryRegistry`| Factory Method | Новый тип курса = новая фабрика и её подключение через `CourseFactoryRegistry`; существующая бизнес-логика и клиентский код не изменяются |
...
Booking-service при получении запроса сохраняет заявку в своей БД со статусом PENDING и в рамках той же транзакции записывает событие в outbox-таблицу; отдельный publisher публикует сообщение в очередь, после чего немедленно отвечает пользователю. Между Booking-service и Restaurants-service вводится очередь сообщений (например, RabbitMQ или Kafka), расположенная в ДЦ1

Отдельный consumer/worker-сервис (в ДЦ1) читает сообщения из очереди и вызывает Restaurants-service по REST с управляемой скоростью (rate limiting / throttling), не превышая его пропускную способность. Для надёжности применяются: idempotency key (защита от дублей при повторной доставке) и retry with backoff + circuit breaker при вызовах Restaurants-service

Аналитики установили, что при равномерной нагрузке сервис ресторанов потреблял бы лишь 20% мощностей. Consumer регулирует темп потребления (rate limiting / throttling), сглаживая пики. Таким образом создаётся механизм backpressure: скорость обращений к сервису ресторанов ограничивается и подстраивается под его пропускную способность, предотвращая перегрузку единственного экземпляра сервиса. Дополнительно контролируются размер очереди, время ожидания сообщений и количество сообщений в DLQ, чтобы вовремя обнаруживать перегрузку или деградацию обработки. Сообщения, которые не удалось обработать после нескольких попыток, помещаются в DLQ для последующего анализа и ручной/автоматизированной повторной обработки. Все заявки, поступившие за 10-15 минут пиковой нагрузки, накапливаются в очереди и обрабатываются в течение нескольких часов - укладываясь в допустимые бизнесом 24 часа. Использование Outbox pattern позволяет избежать ситуации, когда запись о бронировании сохранилась, а сообщение в очередь не отправилось или наоборот. Использование idempotency key позволяет безопасно обрабатывать повторную доставку сообщений и избегать дублирования бронирований
...
flowchart LR
    ...
    B1 -->|Outbox publish| MQ
    B2 -->|Outbox publish| MQ
    BN -->|Outbox publish| MQ
